...
void search_strategy ::twopoint_mutation(Solution *curt_sln)
{
    ...
    for (int i = 0; i < iter; i++)
    {
        ...
        // city1 = rand_int(1, prob->size - 3);
        // city2 = rand_int(city1+1, prob->size - 1);
        while (city1 == city2)
        {
            city2 = rand_int(1, prob->size - 1);
        }
        ...
        else if (city1 == (city2 + 1))
        {
            sub_dist1 = distances[curt_sln->route[city2 - 1].index][curt_sln->route[city2].index] + distances[curt_sln->route[city1 + 1].index][curt_sln->route[city1].index];
            sub_dist2 = distances[curt_sln->route[city2 - 1].index][curt_sln->route[city1].index] + distances[curt_sln->route[city1 + 1].index][curt_sln->route[city2].index];
        }
        else
        {
...
int search_strategy ::check_feasibility_sln(Solution *sln)
{
    ...
    evaluate_solution(sln);
    double delta = d - sln->distance_sum;
    ...
    if (city_visited.size() != prob->size)
    {
        cout << "not all cities are visited.........." << endl;
    }
    return 1;
}
...
void search_strategy ::memetic_algorithm(clock_t start_time, int max_time)
{
    ...
    for (int i = 0; i < max_iter; i++)
    {
        ...
        check_feasibility_sln(prob->best_sln);
